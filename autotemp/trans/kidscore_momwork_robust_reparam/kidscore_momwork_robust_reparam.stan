...
target+=normal_lpdf(kid_score[observe_i]|beta[1]+(beta[2])*(work2[observe_i])+(beta[3])*(work3[observe_i])+(beta[4])*(work4[observe_i]),inv_sqrt(robust_local_tausigma[observe_i])*(sigma));
}
}
